...
import { generatePetPdf } from '../services/pdfExport.js';
...
export default async function petRoutes(fastify) {
  ...
  fastify.get('/pets/:petId/export.pdf', { preHandler: [fastify.verifyPetOwnership] }, async (request, reply) => {
    const buffer = await generatePetPdf(request.params.petId, request.user.id);
    reply
      .header('Content-Type', 'application/pdf')
      .header('Content-Disposition', `attachment; filename="prontuario-${request.params.petId}.pdf"`)
      .send(buffer);
  });
}
