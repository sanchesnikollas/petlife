...
export default function Dashboard() {
  ...
  (food.schedule || []).forEach((time) => {
    upcomingEvents.push({
      ...
      details: `Ração: ${food.brand}\nPorção: ${food.portionGrams}g\nHorário: ${time}`,
      date: today,
      time,
    });
  });
  ...
  vaccines.forEach((v) => {
    ...
    if (isWithinInterval(d, { start: today, end: weekEnd }) || v.status === 'overdue') {
      upcomingEvents.push({
        ...
        details: `Vacina: ${v.name}\nPróximo reforço: ${formatDateShort(v.nextDue)}\nClínica: ${v.clinic || 'Não informada'}\nVeterinário(a): ${v.vet || 'Não informado(a)'}`,
        date: d,
      });
    }
  });
  ...
  medications.filter((m) => m.active).forEach((m) => {
    upcomingEvents.push({
      ...
      details: `Remédio: ${m.name}\nDose: ${m.dose}\nFrequência: ${m.frequency}\nPróxima dose: ${formatDateShort(m.nextDue)}`,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5 stagger-children animate-fade-in-up">
        ...
        <StatusCard
          ...
          label="Ultimo peso"
          value={`${pet.weight} kg`}
          sublabel={weightHistory.at(-1)?.date || ''}
        />
      </div>
      ...
      <div className="mb-5 animate-fade-in-up">
        ...
        <div className="grid grid-cols-4 gap-2 stagger-children">
          {[
            ...
            { icon: Pill, label: 'Medicação', key: 'medication' },
      ...
      <div className="mb-6 animate-fade-in-up">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display text-lg text-text-primary">Próximos 7 dias</h2>
      ...
      <Modal open={modal === 'vaccine'} onClose={() => { setModal(null); setFormData({}); }} title="Registrar Vacina">
        <div className="space-y-3">
          ...
          <div>
            <label className={labelClass}>Próximo reforço *</label>
            <input
              type="date"
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: PetVet Clínica"
              value={formData.clinic || ''}
              onChange={(e) => updateField('clinic', e.target.value)}
            />
          </div>
          <div>
            <label className={labelClass}>Veterinário(a)</label>
            <input
              className={inputClass}
      ...
      {/* Medication Modal */}
      <Modal open={modal === 'medication'} onClose={() => { setModal(null); setFormData({}); }} title="Adicionar Medicação">
        <div className="space-y-3">
          <div>
